...
code = text2code('schnorr_verify1\bytes_schnorr_verify.txt');
codelen=length(code);
...
fdset1=fopen('schnorr_verify1\Set_I_V.bin', 'w');
fdset2=fopen('schnorr_verify1\Set_I_P.bin', 'w');
fdOpList=fopen('schnorr_verify1\OpList.bin', 'w');
fdWireList=fopen('schnorr_verify1\WireList.bin', 'w');
...
for k=1:length(oplist)
    ...
    fdInput=fopen(['schnorr_verify1\instance\Input_opcode' num2str(k-1) '.json'], 'w');
    fdOutput=fopen(['schnorr_verify1\instance\Output_opcode' num2str(k-1) '.json'], 'w');
